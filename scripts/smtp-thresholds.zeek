...
export {
	#global smtp_debug=open_log_file("smtp-debug");  # zeek porting disabled
	...
	global smtp_activity: table[string] of smtp_thresholds &create_expire=4 hrs; # zeek porting &persistent;
	...
	global smtp_subject_activity: table[string] of smtp_subjects
	    &create_expire=4 hrs; # zeek porting  &persistent ;
	...
	global smtp_to_threshold_idx: table[string] of count &default=0
	    &write_expire=1 day &redef;
	...
	global ok_bulk_sender_ip_feed = fmt("%s/feeds/smtp-thresholds::ok_bulk_sender",
	    @DIR) &redef;
...
#if ( n$note == SMTPurl::HighNumberRecepients)
...
function process_rcptto(mailfrom: string, subject: string, rec: set[string])
	...
	for ( rcptto in rec )
		...
		#print smtp_debug,  fmt ("rcptto: %s", rcptto); # zeek porting disable
...
function process_to(mailfrom: string, subject: string, rec: set[string]): bool
	...
	for ( to in rec )
		...
		#print smtp_debug, fmt ("TO_SPLIT : %s", to_split); # zeek porting disabled
...
function check_smtp_thresholds(rec: SMTP::Info)
	...
	if ( |smtp_activity[mailfrom]$msg_origin_ip| > 20 )
		...
		NOTICE([$note=ManyMsgOrigins, $msg=_msg, $suppress_for=6 hrs, $identifier=cat(
		    mailfrom)]);
		}
